...
\begin{itemize}
\item ProcessDebugFlags: Should be 1.
\item ProcessDebugPort: Should be 0.
\item ProcessBasicInformation: Reveals the parent process ID.
\item ProcessBreakOnTermination: Please see \textit{NtSetInformationProcess} in Section~\ref{sec:NtSetInformationProcess_section}.
\item ProcessHandleTracing: Please see \textit{NtSetInformationProcess} in Section~\ref{sec:NtSetInformationProcess_section}.
\end{itemize}
...
This option hooks \textit{NtResumeThread}. If the malware creates a new process, option terminates and dumps the newly created process. If you are unpacking malware, enable and try it. Should be only used inside a Virtual Machine (VM).
